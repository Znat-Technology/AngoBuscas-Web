...
export const fetch = {
  ...
  methods: {
    ...
    async getCount (endpoint) {
      const response = await this.$axios.put(`${process.env.API}${endpoint}/count`, { search: this.modelSearch })
      this.total = response.data
      this.maxPages = parseInt(Math.ceil(this.total / 10))
    },
    async getInfoFromDB () {
      const endpoint = (this.searchPeopleOrDocument === 'Documentos') ? 'document' : 'people'
      this.skeletonAlive = true
      const response = await this.$axios.put(`${process.env.API}${endpoint}/search?page=${this.current}&limit=10`, { search: this.modelSearch })
